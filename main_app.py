...
class DetectorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("DEEPFAKE DETECTOR - HYBRID ENGINE")
    ...
    def _build_ui(self):
        ...
        tk.Label(hdr, text="v3.0 · AI HYBRID FORENSICS", font=("Courier", 9), fg=TEXT_DIM, bg=BG_DARK).place(x=31, y=46)
        ...
        tk.Label(scan_frame, text="ANALYSIS ENGINE", font=("Courier", 8), fg=TEXT_DIM, bg=BG_DARK).place(x=220, y=240)
    ...
    def open_settings(self):
        ...
        gemini_entry = tk.Entry(settings_win, width=45, show="*", bg=BG_PANEL, fg=ACCENT, insertbackground=ACCENT, bd=1, relief="solid", font=("Courier", 10))
        ...
        def on_save():
            ...
            self.log("Gemini API Configuration saved successfully.", "success")

        save_btn = tk.Button(settings_win, text="SAVE CONFIG", font=("Courier", 10, "bold"), fg=BG_DARK, bg=SUCCESS, activebackground="#00CCB0", bd=0, cursor="hand2", command=on_save, pady=5)
    ...
    def _run_gemini_analysis(self, img_path):
        config = load_config()
        api_key = config.get("gemini_api_key", "")
        
        if not api_key:
            self.log("Skipping Deep Scan: Gemini API key not found.", "warn")
            return None, None
    ...
    def process(self, img_path):
        try:
            ...
            self.log("Phase 1: Running Local Rust Engine…")
            ...
            if not os.path.exists(RUST_BINARY):
                ...
                if len(stats) >= 3:
                    ...
                    if not (c00 > 25.0 and c11 > 25.0):
                        rust_verdict = "FAKE"
                        self.log(f"Rust Analysis: FAKE (Texture too smooth, Diff={diff:.2f})")
                    elif diff > 10.0:
                        rust_verdict = "FAKE"
                        self.log(f"Rust Analysis: FAKE (Anisotropic artifacts, Diff={diff:.2f})")
                    else:
                        rust_verdict = "REAL"
                        self.log(f"Rust Analysis: REAL (Natural noise, Diff={diff:.2f})")

            gemini_verdict, explanation = self._run_gemini_analysis(img_path)
            
            final_verdict = rust_verdict
            if gemini_verdict:
                self.log("--- GEMINI FORENSIC REPORT ---", "accent")
